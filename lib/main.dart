...
class _NotepaddAccueilPageState extends State<NotepaddAccueilPage> {
  ...
  Scaffold _buildScaffold(BuildContext context) => Scaffold(
        appBar: buildAppBar(context, widget.titre),
        body: Center(child: _buildMenuColumn(context)),
      );

  Column _buildMenuColumn(BuildContext context) => Column(
...
_buildInkWellSquare(BuildContext context, Menu menu, double size) => InkWell(
    onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context) => menu.lien)),
    child: _buildSquare(context, menu, size));

_buildInkWellRectangle(BuildContext context, Menu menu, double width, double height, String link) => InkWell(
    onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context) => menu.lien)),
    child: _buildRectangle(context, menu, width, height));

_buildSquare(BuildContext context, Menu menu, double size) => _buildStructure(context, menu, size, size);

_buildRectangle(BuildContext context, Menu menu, double width, double height) => _buildStructure(context, menu, width, height);
